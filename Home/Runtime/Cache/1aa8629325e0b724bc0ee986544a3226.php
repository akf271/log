...
<div class="divGlobal margin">
  ...
  <div class="DivMainRight">
    <div class="h80">
      <ul class="breadcrumb ">
        <li class="blank40"></li>
        <li><a href="/pay">充值</a> <span class="divider">/</span></li>
        <li><a style="font-weight:bold" href="/pay/bshoprecord">分店充值记录</a> <!--<span class="divider">/</span></li>
          ...
          <tr>
            ...
            <th class="h20"><div class="dropdown"> <a class="dropdown-toggle" id="dLabel" role="button" data-toggle="dropdown" data-target="#" href="/page.html"> 分店名称 <b class="caret"></b></a>
              <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel" style="overflow:scroll; height:200px;">               
                    <?php if(is_array($bshopArr)): $i = 0; $__LIST__ = $bshopArr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bshopArrData): $mod = ($i % 2 );++$i;?><li><a href="/pay/bshoprecord/id/<?php echo ($bshopArrData["userid"]); ?>"><?php echo ($bshopArrData["shop_name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
              </ul>
            </div></th>
            <th class="h20">充值渠道</th>
            <th class="h20">充值金额</th>
            <th class="h20">状态</th>
          </tr>
        </thead>
        <tbody>
         <?php if(is_array($payInfo)): $i = 0; $__LIST__ = $payInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
            <td><?php echo ($data["order_id"]); ?></td>
            <td><?php echo ($data["pay_start_time"]); ?></td>
            <td><a href="#" style="color:#d9400b;"><?php echo ($data["bshopName"]); ?></a></td>
            <td><?php echo ($data["pay_from"]); ?></td>
            <td><?php echo ($data["money"]); ?></td>
            <td><span><?php echo ($data["pay_status"]); ?></span><!--<span>处理中</span><span>取消</span>--></td>
