...
$(document).ready(function(){		   
	...
	$("#reduceBshopMoneyButton").click(function() {
		$("#reduceBshopMoneyButton").attr("disabled","disabled");
		var url = "/distributemoney/doreducebshopmoney";
		$("#reduceBshopMoneyForm").ajaxSubmit({
			url:url,
			type:"POST",
			success:function(data, st) {
			   $("#reduceBshopMoneyButton").removeAttr("disabled");
			   if (data.result!=0) {
                   //错误
				    art.dialog({
						lock: true,
