...
class Image {
     ...
     * 创建加法验证码
     */
	static function buildAddLogicImageVerify($length=6, $mode=1, $type='png', $width=48, $height=22, $verifyName='logicResultVerify') {
    	import('ORG.Util.String');
        $randval = String::randString($length, $mode);
	 	$nub1 = rand(10,99);
	 	$nub2 = rand(10,99);
	 	$addResult = $nub1 + $nub2;
	 	$randval = $nub1 . "+" . $nub2 . "=";
        session($verifyName, md5($addResult));
        $width = ($length * 10 + 10) > $width ? $length * 10 + 10 : $width;
        if ($type != 'gif' && function_exists('imagecreatetruecolor')) {
            $im = imagecreatetruecolor($width, $height);
        } else {
...
//        }
        for ($i = 0; $i < $length; $i++) {
            imagestring($im, 5, $i * 10 + 5, 3, $randval{$i}, $stringColor);
        }
        Image::output($im, $type);
    }
